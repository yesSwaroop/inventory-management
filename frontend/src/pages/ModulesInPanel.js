...
function ModulesInPanel() {
    const [history, setHistory] = useState([]);
    const location = useLocation()
    const { panelID } = location.state
    useEffect(() => {
        axios.get(`http://localhost:4000/modules-in-panel?panelID=${panelID}`)
            .then((res) => {
                let x = res.data;
                x.map((item) => {
                    for (let key in item) {
                        if (key === 'request_id' && item[key] === null) {
                            item[key] = <Link to='/request-warranty' state={{ moduleID: item['module_id'], panelID: panelID }}><Button _hover={{ bg: 'pink' }} variant='outline' colorScheme='pink' size='xs'>Claim Warranty</Button></Link>
                            item['replaced_date'] = 'Not Replaced';
                        }
                    }
                });
                setHistory(res.data);
    ...
    const columns = React.useMemo(
        () => [
            ...
            {
                Header: "REPLACEMENT ID",
                accessor: "request_id"
            },
            {
                Header: "REPLACED DATE",
                accessor: "replaced_date"
            }
        ],
        []
    );
