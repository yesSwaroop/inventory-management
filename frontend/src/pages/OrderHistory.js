...
function OrderHistory() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:4000/order-history?customerID=${localStorage.getItem('user-id')}`)
      .then((res) => {
        let x = res.data;
        x.map((item)=>{
            for(let key in item) {
              if(key==='order_id') {
                item[key] = <Link to='/panels-in-order' state={{orderID : item[key]}}><Button colorScheme={'whatsapp'}>{item[key]}</Button></Link>
              }
            }
        });
        setHistory(res.data);
